...
export class UserService {
    ...
    async updateById(
        ...
        try {
            const updatedUser = await this.userRepository.updateUserAsync(
                id,
                updateData
            );
            if (!updatedUser) {
    ...
    // Acivates a user account by their ID
    async activateUser(id: string): Promise<ServiceResponse<UserType | null>> {
        try {
            const user = await this.userRepository.findById(id);
            if (!user) {
                return new ServiceResponse(
            ...
            user.isActive = true;
            const activatedUser = await this.userRepository.updateUserAsync(
                id,
                user
            );
            if (!activatedUser) {
            ...
            const errorMessage = `Error activating user with id ${id}: ${(ex as Error).message}`;
            logger.error(errorMessage);
            return new ServiceResponse(
                ResponseStatus.Failed,
                errorMessage,
